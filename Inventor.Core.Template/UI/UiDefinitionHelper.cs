...
public class UiDefinitionHelper
{
        ...
        string toolTip, string iconFolder, string theme)
    {
        ...
        try
        {
            ...
            var btnDef = controlDefs.AddButtonDefinition(displayName, internalName,
    ...
    public static (IPictureDisp iPicDisp16X16, IPictureDisp iPicDisp32X32) GetButtonIcons(string iconFolder, string theme)
    {
        ...
        if (string.IsNullOrEmpty(iconFolder) || !System.IO.Directory.Exists(iconFolder)) return (null, null);
        var filename16X16 = System.IO.Path.Combine(iconFolder, $"16x16{theme}.png");
        var filename32X32 = System.IO.Path.Combine(iconFolder, $"32x32{theme}.png");

        if (System.IO.File.Exists(filename16X16))
        {
            try
            {
                System.Drawing.Bitmap image16X16 = new(filename16X16);
        ...
        if (System.IO.File.Exists(filename32X32))
        {
            try
            {
                System.Drawing.Bitmap image32X32 = new(filename32X32);
